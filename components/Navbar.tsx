...
const Navbar: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const openFromEvent = () => setAccessOpen(true);
    window.addEventListener('cfid:open-client-access', openFromEvent);

    return () => {
      window.removeEventListener('scroll', handleScroll);
      window.removeEventListener('cfid:open-client-access', openFromEvent);
    };
  }, []);
  ...
  return (
    <>
        ...
        <div className="container mx-auto px-4 md:px-6 flex justify-between items-center">
          <a href="#" className="flex items-center gap-2 group">
            <div className="relative">
              <Hexagon
                className="text-brand-primary fill-brand-primary/20 w-8 h-8 group-hover:rotate-180 transition-transform duration-700"
                strokeWidth={1.5}
              />
      ...
      <ClientAccessModal open={accessOpen} onClose={() => setAccessOpen(false)} />
    </>
  );
};
