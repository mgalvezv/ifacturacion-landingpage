      ...
      >
        ...
        <div className="flex flex-col lg:flex-row max-h-[75vh] lg:max-h-[70vh]">
          <div className="flex-1 p-5 sm:p-6 lg:p-8 overflow-y-auto space-y-5 border-b lg:border-b-0 lg:border-r border-slate-700">
            {feature.badge && (
              <span className="inline-flex items-center rounded-md border border-brand-primary/40 bg-brand-primary/10 px-2.5 py-1 text-[11px] font-semibold uppercase tracking-wider text-brand-primary">
                {feature.badge}
              </span>
            )}
            ...
            {feature.operations && feature.operations.length > 0 && (
              <div>
                ...
                <ul className="space-y-2.5">
                  {feature.operations.map((item) => (
                    <li
                      key={item}
                      className="flex items-start gap-2.5 text-sm text-slate-300"
                    >
                      <span className="text-brand-tech mt-0.5 shrink-0">→</span>
                      <span>{item}</span>
                    </li>
                  ))}
                </ul>
